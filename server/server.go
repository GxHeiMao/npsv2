...
func StartNewServer(bridgePort int, cnf *file.Tunnel, bridgeType string, bridgeDisconnect int) {
	...
	go DealBridgeTask()
	go dealClientFlow()
	go user.InitUserList()
	go syncHistoryLoginIps()
...
func syncHistoryLoginIps() {
	ticker := time.NewTicker(time.Minute)
	defer ticker.Stop()
	for {
		select {
		case <-ticker.C:
			userStr, _ := json.Marshal(user.GlobalUserList)
			path := filepath.Join(common.GetRunPath(), "conf", "user.json")
			fileUser, err := os.Create(path + ".tmp")
			fileUser.Write(userStr)
			_ = fileUser.Sync()
			_ = fileUser.Close()
			err = os.Rename(path+".tmp", path)
			if err != nil {
				logs.Error(err, "user store to file err, data will lost")
			}
			for _, account := range user.GlobalUserList {
				loginArr := make(map[string]int64)
				historyLoginArr := make(map[string]int64)
				json.Unmarshal([]byte(account.LoginIps), &loginArr)
				json.Unmarshal([]byte(account.HistoryLoginIps), &historyLoginArr)
				if len(loginArr) > 0 {
					for ip, i := range loginArr {
						if i+180 <= time.Now().Unix() {
							//代表本次连接过期
							historyLoginArr[ip] = i
							delete(loginArr, ip)
						}
					}
				}
				hisStr, _ := json.Marshal(historyLoginArr)
				account.HistoryLoginIps = string(hisStr)
				loginStr, _ := json.Marshal(loginArr)
				account.LoginIps = string(loginStr)
			}
		}
	}
}
